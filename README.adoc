...
#TOURE Aboubacar# et #BELLEGRADE Angelo Garielle#
...
Non, le code n'est pas structuré en couches, mais utilise plustôt un modèle MVC. 
C'est possible de rencontrer des problèmes de maintenabilité et d’évolutivité à long terme sans une structure de couche préalablement définie.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

Dans la classe TodoListController

*Cette règle est-elle facilement testable par un test unitaire ?*

Oui en utilisant 2 tests unitaires le premier ajout de ['LATE!'] si le TodoItem à plus 24h et le second s'assurer que la méthode finalContent n'est pas exécuter si le TodoItem est à moins 24h.
...
Cette demarre un nouveau conteneur Docker de l'image postgres

*Les options*

**-it** :  lance le conteneur en mode interacif avec un terminal

**-e POSTGRES_PASSWORD=password** : definie la variable d'environnement POSTGRES_PASSWORD avec la valeur password

**-p 5432:5432** :  mappe le port 5432 du conteneur au port 5432 de la machine hôte
...
La meilleure façon est de creer un service metier de l'injecter est d'utiliser l'annotation `@Auwired` 
